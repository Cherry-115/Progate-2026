...
class CreatePaymentRequest {
  ...
  /// smallest currency unit (e.g. JPY)
  ///
  /// Minimum value: 1
  int amount;
  ...
  @override
  String toString() => 'CreatePaymentRequest[amount=$amount, currency=$currency, metadata=$metadata]';
  ...
  static CreatePaymentRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'amount'), 'Required key "CreatePaymentRequest[amount]" is missing from JSON.');
        assert(json[r'amount'] != null, 'Required key "CreatePaymentRequest[amount]" has a null value in JSON.');
        assert(json.containsKey(r'currency'), 'Required key "CreatePaymentRequest[currency]" is missing from JSON.');
        assert(json[r'currency'] != null, 'Required key "CreatePaymentRequest[currency]" has a null value in JSON.');
        return true;
      }());
